...
def create_temporal_features(df):
    ...
    # 3. Crear Features de Fourier (La "Sinuide") con armónicos adicionales
    # Esto convierte el mes (1-12) en un círculo perfecto
    # Primer armónico (anual)
    df_copy['month_sin_1'] = np.sin(2 * np.pi * df_copy['start_month'] / 12)
    df_copy['month_cos_1'] = np.cos(2 * np.pi * df_copy['start_month'] / 12)
    # Segundo armónico (semestral - captura patrones de 6 meses)
    df_copy['month_sin_2'] = np.sin(4 * np.pi * df_copy['start_month'] / 12)
    df_copy['month_cos_2'] = np.cos(4 * np.pi * df_copy['start_month'] / 12)
    # Tercer armónico (trimestral - captura patrones de 3 meses)
    df_copy['month_sin_3'] = np.sin(6 * np.pi * df_copy['start_month'] / 12)
    df_copy['month_cos_3'] = np.cos(6 * np.pi * df_copy['start_month'] / 12)
...
NUMERIC_FEATURES = [
    ...
    'month_sin_1', 'month_cos_1',     # <-- Feature de Fourier (anual)
    'month_sin_2', 'month_cos_2',     # <-- Feature de Fourier (semestral)
    'month_sin_3', 'month_cos_3',     # <-- Feature de Fourier (trimestral)
...
embedding_pipeline_pca = Pipeline(steps=[
    ...
    ('scaler', StandardScaler()),
    ('pca', PCA(n_components=60, random_state=42)) # 60 componentes para máxima información
])
...
# Modelo 1: LightGBM (hiper-optimizado para Kaggle)
lgbm_model = lgb.LGBMRegressor(
    objective='quantile',
    alpha=0.87,         # Percentil 87 (más agresivo contra ventas perdidas)
    n_estimators=2500,  # Más iteraciones para convergencia óptima
    learning_rate=0.018, # Learning rate aún más bajo para ajuste fino
    num_leaves=60,      # Más hojas para capturar patrones complejos
    max_depth=11,       # Mayor profundidad
    min_child_samples=12, # Más flexible
    subsample=0.87,     # Más datos por iteración
    colsample_bytree=0.87,
    reg_alpha=0.03,     # Regularización más suave
    reg_lambda=0.03,
    n_jobs=-1,
    random_state=42,
    verbose=-1
...
catboost_model = CatBoostRegressor(
    iterations=2500,    # Más iteraciones para convergencia óptima
    learning_rate=0.018, # Learning rate más bajo
    depth=10,           # Mayor profundidad
    l2_leaf_reg=2.0,    # Regularización más suave
    loss_function='Quantile:alpha=0.87', # Mismo objetivo agresivo que LGBM
    eval_metric='MAE',
    random_seed=42,
    verbose=0,
...
# Ensamble final (optimizado: más peso a CatBoost)
final_predictions = 0.25*final_preds_lgbm + 0.75*final_preds_catboost

# Post-procesamiento final (asegurar que no haya demanda negativa)
final_predictions[final_predictions < 0] = 0

# Ajuste más agresivo para compensar ventas perdidas (5%)
...
# Guardar archivo
submission_filename = 'submission_GroupKFold_Ensemble_Fourier_Ultra_v2.csv'
